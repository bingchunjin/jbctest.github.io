...
- **Q：Factory分区的值可以修改吗？通过什么方式进行修改？**
  A：此分区是可以在串口通过指令进行修改的，
     ...
     分区具体参考[FLASH分区开发手册](https://jbctest.github.io/2020/09/08/flashPartitionGuide/)
...
- **Q：软件LAN WAN划分如何实现的？**
  A: 编译镜像前在/openwrt-18.06 /target/linux/siflower/sf19a28-fullmask/base-files-SF19A28-版型/etc/board.d路径下修改 01_network 文件中完成划分
     参考文档[有线网络和服务介绍](https://jbctest.github.io/2020/09/08/ethernetGuide/)
...
- **Q：系统下/etc/config/目录下各个配置文件的作用是什么？**
  A：参考[config文件配置手册](https://jbctest.github.io/2020/09/11/config_setting/)
...
- **Q：使用nand flash的产品uboot和openwrt该如何编译？**
  A：	参考[SPI Nand Flash物料调试指南](https://jbctest.github.io/2021/09/23/spi_nand_flash/#spi-nand-flash%E7%89%A9%E6%96%99%E8%B0%83%E8%AF%95%E6%8C%87%E5%8D%97)
...
- **Q：为什么wifi灯通过节点方式控制不了？需要如何控制？**
  A：wifi的灯并不是控制不了，而是当前在sys/class/leds下的节点是wifi驱动注册实现的，四个节点分别检测节点名称对应的phy的数据包，有数据就闪一下。修改单独一个节点就可能会出现无法控制或常亮的情况。
     ...
     2、将dts中wifi的gpio删去
     3、按照文档[[GPIO综合使用手册](https://jbctest.github.io/2020/07/30/gpio_guide/) ，添加一套自己的led节点（记得gpio口要配置正确，可参考之前删去的wifi的gpio）
...
- **Q：siflower性能如何测试？有无指导文档？**
  A：性能测试说明及方案参考https://jbctest.github.io/2020/11/30/siflower_throughput_guide/

&emsp;
- **Q：射频性能标定相关说明文档？**
  A：射频性能测试参考https://jbctest.github.io/2020/12/01/siflower_ate_test_guide/
